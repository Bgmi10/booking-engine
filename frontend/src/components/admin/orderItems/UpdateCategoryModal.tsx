...
import { RiCloseLine, RiSearchLine, RiImageAddLine } from "react-icons/ri";
import { BiLoader } from "react-icons/bi";
import { baseUrl } from "../../../utils/constants";
import { useImageUpload } from "../../../hooks/useImageUpload";
import type { Category, OrderItem } from "../../../types/types";
...
export default function UpdateCategoryModal({
  ...
  const [availabilityRule, setAvailabilityRule] = useState<any>(null);
  ...
  // Effect to initialize form state when the category changes
  useEffect(() => {
    if (category) {
      ...
      if (category.availabilityRule) {
        setAvailabilityRule(category.availabilityRule);
        setHasAvailabilityRule(true);
      } else {
        setAvailabilityRule({
          name: "All Week",
          startTime: "00:00",
        ...
        setHasAvailabilityRule(false);
      }

      if (category.imageUrl) {
        setInitialImages([category.imageUrl]);
      } else {
        resetImages();
      }
    }
  }, [category, setInitialImages, resetImages]);

  // Effect to fetch all products when the modal opens
  useEffect(() => {
    if (isOpen) {
  ...
  const updateRule = (field: string, value: any) => {
    setAvailabilityRule((prev: any) => ({ ...prev, [field]: value }))
  }

  const toggleDay = (day: number) => {
    if (!availabilityRule) return;
    const days = availabilityRule.daysOfWeek.includes(day)
      ? availabilityRule.daysOfWeek.filter((d: number) => d !== day)
              ...
              <div className="col-span-1 border-t pt-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="flex items-center">
                    <label htmlFor="hasRule-update" className="mr-2 text-sm font-medium text-gray-700">
                      Enable Rule
                    </label>
                    <input
                      type="checkbox"
                ...
                {hasAvailabilityRule && availabilityRule && (
              ...
              <div className="flex-grow overflow-y-auto border-t border-gray-200 -mx-4 px-4 pt-2">
                {filteredProducts.length > 0 ? (
                  filteredProducts.map((product) => (
                    ...
                    >
                      <label
                        htmlFor={`product-update-${product.id}`}
