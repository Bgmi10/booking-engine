...
export default function UpdateEnhancementRuleModal({ 
  isOpen, 
  onClose, 
  onSuccess, 
  rule 
}: UpdateEnhancementRuleModalProps) {
  // Form states
  const [name, setName] = useState(rule.name)
  const [selectedEnhancementId, setSelectedEnhancementId] = useState(rule.enhancementId)
  const [availabilityType, setAvailabilityType] = useState<"ALWAYS" | "WEEKLY" | "SPECIFIC_DATES" | "SEASONAL">(
    rule.availabilityType as any
  )
  const [availableDays, setAvailableDays] = useState<string[]>(rule.availableDays || [])
  ...
  const [seasonEnd, setSeasonEnd] = useState<Date | null>(
    rule.seasonEnd ? new Date(rule.seasonEnd) : null
  )
  const [validFrom, setValidFrom] = useState<Date | null>(
    rule.validFrom ? new Date(rule.validFrom) : null
  )
  const [validUntil, setValidUntil] = useState<Date | null>(
    rule.validUntil ? new Date(rule.validUntil) : null
  )
  const [roomScope, setRoomScope] = useState<"ALL_ROOMS" | "SPECIFIC_ROOMS">(rule.roomScope as any)
  const [roomIds, setRoomIds] = useState<string[]>(rule.roomIds || [])
  ...
  } = useRooms({ showToastOnError: false })
  ...
  const loadingData = loadingEnhancements || loadingRooms

  // Reset form to original values
  useEffect(() => {
    if (isOpen && rule) {
      setName(rule.name)
      setSelectedEnhancementId(rule.enhancementId)
      setAvailabilityType(rule.availabilityType as any)
      setAvailableDays(rule.availableDays || [])
      ...
      setRoomScope(rule.roomScope as any)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${baseUrl}/admin/enhancement-rules/${rule.id}`, {
        method: "PUT",
        credentials: "include",
        headers: {
              ...
              <div>
                ...
                <div className="flex items-center">
                  ...
                  <label htmlFor="isActive" className="ml-2 block text-sm text-gray-900">
                    Active (Rule will be applied to bookings)
                  </label>
                </div>
              </div>
            </div>
