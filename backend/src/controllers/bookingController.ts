...
export const createCheckoutSession = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    // Enhancement line items (if any)
    for (const enh of enhancements) {
      const enhancement = await prisma.enhancement.findUnique({ where: { id: enh.enhancementId } });
      ...
      if (enhancement) {
        line_items.push({
          price_data: {
            ...
            unit_amount: Math.floor(enhancement.price * 100),
          },
